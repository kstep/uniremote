...
fn special(_lua: &Lua, _csidl: String) -> Result<String> {
    ...
    Err(Error::runtime("special folders are not supported (Windows only)"))
}
...
fn move_path(_lua: &Lua, (source, destination): (String, String)) -> Result<()> {
    fs::rename(&source, &destination)
        .map_err(|error| Error::runtime(format!("failed to move '{source}' to '{destination}': {error}")))?;
    Ok(())
}
...
fn delete(_lua: &Lua, (path, recursive): (String, Option<bool>)) -> Result<()> {
    ...
    if path.is_file() {
        fs::remove_file(path).map_err(|error| {
            Error::runtime(format!("failed to delete file '{}': {error}", path.display()))
        })?;
    } else if path.is_dir() {
        if recursive {
...
fn files(_lua: &Lua, (path, hidden): (String, Option<bool>)) -> Result<Vec<String>> {
    ...
    for entry in entries {
        let entry = entry.map_err(|error| {
            Error::runtime(format!("failed to read directory entry: {error}"))
        })?;
...
fn size(_lua: &Lua, path: String) -> Result<u64> {
    ...
    if metadata.is_dir() {
        ...
        for entry in walkdir::WalkDir::new(path) {
            let entry = entry.map_err(|error| {
                Error::runtime(format!("failed to walk directory: {error}"))
            })?;
            if entry.file_type().is_file() {
                total_size += entry.metadata().map_err(|error| {
...
fn created(_lua: &Lua, path: String) -> Result<u64> {
    ...
    let duration = created.duration_since(UNIX_EPOCH).map_err(|error| {
        Error::runtime(format!("invalid creation time: {error}"))
    })?;
...
fn modified(_lua: &Lua, path: String) -> Result<u64> {
    ...
    let duration = modified.duration_since(UNIX_EPOCH).map_err(|error| {
        Error::runtime(format!("invalid modification time: {error}"))
    })?;
...
mod tests {
    ...
    fn test_fs_exists() {
        ...
        load(&lua, &libs).unwrap();
        lua.globals().set("libs", libs).unwrap();
        lua.globals().set("existing_path", test_file.display().to_string()).unwrap();
        lua.globals().set("nonexistent_path", temp_dir.path().join("nonexistent.txt").display().to_string()).unwrap();
    ...
    fn test_fs_isfile_isdir() {
        ...
        load(&lua, &libs).unwrap();
        lua.globals().set("libs", libs).unwrap();
        lua.globals().set("file_path", test_file.display().to_string()).unwrap();
        lua.globals().set("dir_path", temp_dir.path().display().to_string()).unwrap();
    ...
    fn test_fs_createdir() {
        ...
        load(&lua, &libs).unwrap();
        lua.globals().set("libs", libs).unwrap();
        lua.globals().set("new_dir_path", new_dir.display().to_string()).unwrap();
    ...
    fn test_fs_readlines_writelines() {
        ...
        load(&lua, &libs).unwrap();
        lua.globals().set("libs", libs).unwrap();
        lua.globals().set("test_path", test_file.display().to_string()).unwrap();
